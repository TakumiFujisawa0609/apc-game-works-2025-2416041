...
void GameScene::GameInit(void)
{
	...
	// 弾の初期化
	for (auto& b : bullet) { // もし既に残っている弾があれば削除
		b.Release();
	}
...
void GameScene::Update(void)
{
	...
	const int bullet_interval = 12; //弾発射間隔

	if (frame < SHOT_INTERVAL)
	{
		//弾発射(12フレームごとに弾を発射）
		if (frame % bullet_interval == 0) {
			for (auto& b : bullet/*弾倉*/) {
				if (!b.isActive) {	//発射
					b.bPos;		//発射位置
					//敵の座標
		...
		//放射状弾
		constexpr float radiality = 12.0f; //自機狙い弾の角度のばらつき
		constexpr float diff_angle = 2.0f * DX_PI_F / radiality; //角度のばらつき
		if (frame % bullet_interval == 0) {
			int cnt = 0;
			float angle = 0.0f; //角度
			for (auto& b : bullet/*弾倉*/) {
